...
def main(args):
    ...
    if args.nestedclaim:
        ...
        for item in args.nestedclaim:
            if (len(item) > 2):
                nestedclaimname = item[0]
                k = item[1]
                v = item[2:]
                if len(v) == 1:
                    v = v[0]
                nested[k] = v
        payload[nestedclaimname] = nested
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        action='append',
        nargs='+',
        help="Nested claim in format parentkey key value1 [value2 ...], only string values are supported. "
             "Multiple nested claims can be specified, e.g., -nestedclaim parentkey key1 val2 val3 -nestedclaim key2 val3 val4."
    )
